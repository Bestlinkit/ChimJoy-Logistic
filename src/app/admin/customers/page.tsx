...
import { Users, Search, ShieldAlert, Star, DollarSign, CalendarCheck, Phone, Mail, Ban, CheckCircle2, Trash2 } from 'lucide-react';
import { subscribeToCustomers, updateCustomerStatusInDb, deleteCustomerFromDb } from '@/lib/firebase/services/admin-db-service';
...
export default function AdminCustomersPage() {
  ...
  const handleDeleteCustomer = async (c: AdminCustomer) => {
    if (!confirm(`Are you sure you want to delete customer ${c.name}? This action cannot be undone.`)) return;
    try {
      await deleteCustomerFromDb(c.id);
      if (adminUser) {
        await logAdminAction(adminUser.email, adminUser.role, 'DELETE_CUSTOMER', 'Customers', `Deleted customer ${c.name}`);
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl border border-[#0B192C]/10 shadow-sm overflow-hidden">
        ...
        <div className="p-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-slate-600">
          <span>
            Page <strong>{currentPage}</strong> of <strong>{totalPages}</strong> ({filtered.length} Total Customers)
          </span>
