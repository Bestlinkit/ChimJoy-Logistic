      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-4">
            <div className="inline-flex flex-wrap gap-1 bg-[#0B192C] p-1.5 rounded-2xl">
              {[
                { id: 'airport', label: 'Airport Pickup' },
                { id: 'within', label: 'Within Owerri' },
                { id: 'outside', label: 'Outside Owerri' },
                { id: 'corporate', label: 'Corporate Booking' },
              ].map((tab) => (
                <button
                  key={tab.id}
                  type="button"
                  onClick={() => setActiveTab(tab.id as any)}
                  className={`py-2 px-4 rounded-xl text-xs font-black uppercase tracking-wider transition-all text-center ${
                    activeTab === tab.id
            ...
            <span className="text-xs font-extrabold text-[#003366] bg-[#003366]/10 px-3 py-1.5 rounded-full hidden md:inline-block">
              ✓ All Bookings Include Professional Chchauffeur
            </span>
          </div>

          {/* Compact Multi-Column Single Row Input Matrix */}
          <form onSubmit={handleBookingSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
              {/* Pickup Location */}
              <div className="space-y-1">
                <label className="font-extrabold uppercase tracking-wider text-[#0E1726] text-[10px]">Pickup</label>
                <div className="flex items-center gap-2 bg-[#F4F6F9] border border-[#0B192C]/15 rounded-xl p-3 focus-within:border-[#003366] focus-within:bg-white transition-colors">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent font-bold text-[#0E1726] focus:outline-none placeholder:text-slate-400"
                  />
                </div>
              </div>
              ...
              {/* Date & Time Combo */}
              <div className="space-y-1">
                <label className="font-extrabold uppercase tracking-wider text-[#0E1726] text-[10px]">Date & Time</label>
                <div className="flex items-center gap-1.5 bg-[#F4F6F9] border border-[#0B192C]/15 rounded-xl p-3">
                  <Calendar className="w-3.5 h-3.5 text-[#0E1726] shrink-0" />
                  <input
                    type="date"
                    required
                    value={pickupDate}
                  ...
                  <input
                    ...
                    className="w-20 bg-transparent font-bold text-[#0E1726] focus:outline-none text-[11px]"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 bg-[#F4F6F9] border border-[#0B192C]/15 rounded-xl p-3">
                  <Car className="w-4 h-4 text-[#0E1726] shrink-0" />
                  <select
                    value={vehicleType}
                    onChange={(e) => setVehicleType(e.target.value)}
                    className="w-full bg-transparent font-bold text-[#0E1726] focus:outline-none cursor-pointer"
                  >
              ...
              <div className="flex items-end">
                <LuxuryButton
                  variant="lemon"
                  size="md"
                  className="w-full justify-center py-3.5 text-[#0B192C]"
